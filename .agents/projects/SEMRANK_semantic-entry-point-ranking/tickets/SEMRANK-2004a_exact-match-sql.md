...
## Acceptance Criteria
- [ ] SQL CASE statement for exact match detection implemented
- [ ] Case-insensitive comparison: `LOWER(c.symbol_name) = LOWER($normalized_query)`
- [ ] Exact match multiplier: 3.0× when match, 1.0 otherwise
- [ ] Null symbol_name handled: Returns 1.0 (no boost, no crash)
- [ ] SQL compiles and executes without errors
- [ ] Separate `exact_mult` column computed for debug visibility
- [ ] $normalized_query parameter added to SQL query

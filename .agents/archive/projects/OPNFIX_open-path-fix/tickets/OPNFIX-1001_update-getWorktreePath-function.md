...
Current implementation uses `LIMIT 1` without ORDER BY (non-deterministic), trusts database data blindly without filesystem validation, and fails when first match has incorrect abs_path. Bug identified in `.agents/reports/2025-11-18_maproom-mcp-context-tool-failure-analysis.md`.

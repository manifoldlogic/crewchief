...
This security threat was identified in `.crewchief/projects/OPNFIX_open-path-fix/planning/security-review.md` as **Threat 3: Symlink Escape**. While symlinks within a repository are legitimate use cases (e.g., linking to shared configuration files), symlinks that escape repository boundaries must be blocked.

...
export function registerCompetitionCommands(program: Command): void {
  ...
  comp
    ...
    .action(async (worktrees: string[], opts: { criteria?: string; base: string; json?: boolean; autoMerge?: boolean; target: string; strategy: 'squash' | 'ff' | 'cherry-pick'; force?: boolean }) => {
      try {
        ...
        if (dirty.length > 0 && !opts.force) {
          ...
          logger.error(`Aborting: uncommitted changes detected in worktrees. Commit or stash first, or re-run with --force.\n${list}`);
          process.exitCode = 1;
          return;
        }
        ...
        if (opts.autoMerge) {
          ...
          const res = await merge.merge({ sourceBranch: winnerBranch, targetBranch: opts.target, strategy: opts.strategy });
          if (res.success) {
...
function buildEvalPrompt(diffs: { branch: string; stat: string; patch: string }[], base: string, criteria?: string): string {

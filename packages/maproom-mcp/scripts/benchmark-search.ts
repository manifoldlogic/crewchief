 ...
 */
function executeSearch(query: string): { result: SearchResult; latencyMs: number } {
  ...
  try {
    const searchResult: SearchResult = JSON.parse(result.stdout)
 ...
 */
function analyzeRanking(hits: SearchHit[]): {
  top_3_kinds: string
  implementation_rank: number | null
  test_rank: number | null
  doc_rank: number | null
 ...
 */
async function getQueryPlan(client: Client, query: string): Promise<string> {
...
async function main() {
  ...
  const csvRows = results.map(r =>
    `"${r.query}","${r.description}",${r.latency_p50_ms},${r.latency_p95_ms},${r.latency_p99_ms},"${r.top_3_kinds}",${r.implementation_rank ?? ''},${r.test_rank ?? ''},${r.doc_rank ?? ''}`
  ).join('\n')
  ...
  const implBeforeTest = results.filter(r =>
    r.implementation_rank !== null &&
    r.test_rank !== null &&
    r.implementation_rank < r.test_rank
  ).length

  const testBeforeImpl = results.filter(r =>
    r.implementation_rank !== null &&
    r.test_rank !== null &&
    r.test_rank < r.implementation_rank
  ).length

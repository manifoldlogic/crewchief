...
Section two content."#;
    ...
    assert_eq!(main_title.start_line, 1, "Main Title should start at line 1");
    assert_eq!(main_title.end_line, 11, "Main Title should end at line 11 (EOF)");
    ...
    assert_eq!(section_one.start_line, 5, "Section One should start at line 5");
    assert_eq!(section_one.end_line, 8, "Section One should end at line 8 (before Section Two)");
    ...
    assert_eq!(section_two.start_line, 9, "Section Two should start at line 9");
    assert_eq!(section_two.end_line, 11, "Section Two should end at line 11 (EOF)");
}
...
Section two content."#;
    ...
    assert_eq!(section_one.end_line, 12, "Section One should include subsection and end before Section Two");

    // Find Subsection 1.1 (h3)
    let subsection = chunks.iter()
    ...
    assert_eq!(subsection.end_line, 12, "h3 should end when parent section ends");
...
Other content."#;
    ...
    assert!(main_section.end_line >= next_section.end_line,
    ...
    assert_eq!(headings, 2, "Should only extract 2 real headings, not the ones in code blocks");
}
...
Another L2 content."#;
    ...
    assert!(l2.end_line < another_l2.start_line,
        "First L2 should end before Another L2 starts");
    ...
    assert!(l3.end_line <= l2.end_line, "L3 should be within L2's section");
    ...
    assert!(l4.end_line <= l3.end_line, "L4 should be within L3's section");
}
...
Final line."#;
    ...
    assert_eq!(third.end_line, total_lines as i32,
...
## Empty Three"#;
    ...
    assert!(empty_one.end_line > 1, "Section should have end_line > start_line");

    // Find Empty Two
    let empty_two = chunks.iter()
    ...
    assert!(empty_two.end_line < has_content.start_line || empty_two.end_line >= has_content.end_line,
...
More content."#;
    ...
    assert!(first_doc.end_line < second_doc.start_line,
        "First h1 section should end when next h1 starts");

    // Second Document should extend to EOF
    let total_lines = source.lines().count();
    assert_eq!(second_doc.end_line, total_lines as i32,

...
fn test_django_models_parsing() {
    ...
    if let Some(user) = user_model {
        ...
        if let Some(sig) = &user.signature {
            assert!(sig.contains("AbstractUser"), "Should capture AbstractUser inheritance");
        }
    }

    // Should extract nested Meta class
    let meta_class = chunks.iter()
    ...
    assert!(get_full_name.is_some(), "Should extract get_full_name method");

    // Should extract Product model with choices
    let product_model = chunks.iter()
    ...
    let status_constants = chunks.iter()
    ...
    let review_model = chunks.iter()
        .find(|c| c.symbol_name == Some("Review".to_string()) && c.kind == "class");
    assert!(review_model.is_some(), "Should extract Review model");

    // Overall count check
    assert!(chunks.len() >= 30, "Should extract many symbols from Django models file (got {})", chunks.len());
}
...
fn test_django_views_parsing() {
    ...
    let product_list_view = chunks.iter()
        .find(|c| c.symbol_name == Some("ProductListView".to_string()) && c.kind == "class");
    assert!(product_list_view.is_some(), "Should extract ProductListView class");

    if let Some(plv) = product_list_view {
        if let Some(sig) = &plv.signature {
            assert!(sig.contains("ListView"), "Should capture ListView inheritance");
        }
    }

    // Should extract view methods
    let get_queryset = chunks.iter()
        .filter(|c| c.symbol_name == Some("get_queryset".to_string()))
        .count();
    assert!(get_queryset >= 2, "Should extract multiple get_queryset methods");

    let get_context_data = chunks.iter()
        .filter(|c| c.symbol_name == Some("get_context_data".to_string()))
        .count();
    assert!(get_context_data >= 2, "Should extract multiple get_context_data methods");

    // Should extract DetailView
    let product_detail = chunks.iter()
    ...
    let product_create = chunks.iter()
    ...
    let add_review = chunks.iter()
        .find(|c| c.symbol_name == Some("add_review".to_string()));
    if let Some(review_fn) = add_review {
        assert!(review_fn.metadata.is_some(), "Decorated view should have metadata");
    }

    // Should extract API view
    let api_view = chunks.iter()
    ...
    let handler404 = chunks.iter()
        .find(|c| c.symbol_name == Some("handler404".to_string()));
    assert!(handler404.is_some(), "Should extract error handler function");

    // Overall count
    assert!(chunks.len() >= 25, "Should extract many symbols from Django views file (got {})", chunks.len());
}
...
fn test_django_urls_parsing() {
    ...
    let product_patterns = chunks.iter()
        .find(|c| c.symbol_name == Some("product_patterns".to_string()));
    assert!(product_patterns.is_some(), "Should extract product_patterns list");

    let urlpatterns = chunks.iter()
...
fn test_flask_app_parsing() {
    ...
    if let Some(user) = user_model {
        ...
        if let Some(sig) = &user.signature {
            assert!(sig.contains("UserMixin"), "Should capture UserMixin inheritance");
            assert!(sig.contains("db.Model"), "Should capture db.Model inheritance");
        }
    }

    // Should extract model methods
    let set_password = chunks.iter()
        .find(|c| c.symbol_name == Some("set_password".to_string()));
    assert!(set_password.is_some(), "Should extract set_password method");

    let check_password = chunks.iter()
        .find(|c| c.symbol_name == Some("check_password".to_string()));
    assert!(check_password.is_some(), "Should extract check_password method");

    // Should extract Post model
    let post_model = chunks.iter()
    ...
    let post_tags = chunks.iter()
        .find(|c| c.symbol_name == Some("post_tags".to_string()));
    assert!(post_tags.is_some(), "Should extract post_tags association table");

    // Should extract decorated functions
    let load_user = chunks.iter()
        .find(|c| c.symbol_name == Some("load_user".to_string()));
    assert!(load_user.is_some(), "Should extract load_user decorated function");

    // Should extract custom decorators
    let admin_required = chunks.iter()
        .find(|c| c.symbol_name == Some("admin_required".to_string()));
    assert!(admin_required.is_some(), "Should extract admin_required decorator");

    // Should extract route handlers
    let index_route = chunks.iter()
        .find(|c| c.symbol_name == Some("index".to_string()));
    assert!(index_route.is_some(), "Should extract index route");

    let register_route = chunks.iter()
...
fn test_flask_blueprints_parsing() {
    ...
    let list_users = chunks.iter()
        .find(|c| c.symbol_name == Some("list_users".to_string()));
    if let Some(route) = list_users {
        assert!(route.metadata.is_some(), "Decorated route should have metadata");
    }

    // Should extract admin_required decorator
    let admin_required = chunks.iter()
        .find(|c| c.symbol_name == Some("admin_required".to_string()));
    assert!(admin_required.is_some(), "Should extract admin_required decorator function");

    // Should extract before_request hook
    let before_request = chunks.iter()
        .find(|c| c.symbol_name == Some("before_request".to_string()));
    assert!(before_request.is_some(), "Should extract before_request hook");

    // Should extract register_blueprints helper
    let register_blueprints = chunks.iter()
        .find(|c| c.symbol_name == Some("register_blueprints".to_string()));
    assert!(register_blueprints.is_some(), "Should extract register_blueprints function");

    // Overall count
    assert!(chunks.len() >= 25, "Should extract many symbols from Flask blueprints file (got {})", chunks.len());
}
...
fn test_flask_extensions_parsing() {
    ...
    let login_manager = chunks.iter()
        .find(|c| c.symbol_name == Some("login_manager".to_string()));
    assert!(login_manager.is_some(), "Should extract login_manager extension");

    let mail = chunks.iter()
        .find(|c| c.symbol_name == Some("mail".to_string()));
    assert!(mail.is_some(), "Should extract mail extension");

    // Should extract init function
    let init_extensions = chunks.iter()
        .find(|c| c.symbol_name == Some("init_extensions".to_string()));
    assert!(init_extensions.is_some(), "Should extract init_extensions function");

    if let Some(init_fn) = init_extensions {
        assert!(init_fn.docstring.is_some(), "init_extensions should have Google-style docstring");
        let docstring = init_fn.docstring.as_ref().unwrap();
        assert!(docstring.contains("Args:") || docstring.contains("Parameters:"),
               "Docstring should have parameters section");
    }

    // Should extract configuration function
    let configure_logging = chunks.iter()
        .find(|c| c.symbol_name == Some("configure_logging".to_string()));
    assert!(configure_logging.is_some(), "Should extract configure_logging function");
    ...
    let clear_all = chunks.iter()
        .find(|c| c.symbol_name == Some("clear_all".to_string()));
    assert!(clear_all.is_some(), "Should extract clear_all static method");

    // Should extract DatabaseManager class
    let db_manager = chunks.iter()
    ...
    // Overall count
    assert!(chunks.len() >= 30, "Should extract many symbols from Flask extensions file (got {})", chunks.len());
}
...
fn test_all_real_world_fixtures_no_panic() {
    ...
    for fixture_path in fixtures {
        ...
        assert!(!chunks.is_empty(),
               "Fixture {} should extract at least some symbols", fixture_path);

        println!("✓ {} parsed successfully ({} symbols)", fixture_path, chunks.len());
    }
}
...
fn test_real_world_chunk_quality() {
    ...
    for class in &classes {
        ...
        assert!(class.end_line >= class.start_line, "Class should have valid end line");

        // Should have signature or docstring (SymbolChunk no longer has preview/ts_doc fields)
        assert!(class.signature.is_some() || class.docstring.is_some(), "Class should have signature or docstring");
    }
    ...
    for method in &methods {
        ...
        assert!(method.end_line >= method.start_line, "Method should have valid end line");

        // Should have signature or docstring (SymbolChunk no longer has preview field)
        assert!(method.signature.is_some() || method.docstring.is_some(), "Method should have signature or docstring");
    }
}

...
fn test_parse_real_readme() {
    ...
    if let Ok(content) = fs::read_to_string(readme_path) {
        ...
        println!("README.md: Extracted {} chunks ({} headings)", chunks.len(), headings);
    } else {
...
fn test_parse_real_claude_md() {
    ...
    if let Ok(content) = fs::read_to_string(claude_md_path) {
        ...
        // Should successfully parse without panicking
        assert!(chunks.len() > 0, "Should extract some chunks from CLAUDE.md");
        ...
        println!("CLAUDE.md: Extracted {} chunks ({} headings, {} code blocks)",
            chunks.len(), headings, code_blocks);
    } else {
...
fn test_parse_real_architecture_doc() {
    ...
    if let Ok(content) = fs::read_to_string(arch_doc_path) {
        ...
        // Should successfully parse without panicking
        assert!(chunks.len() > 0, "Should extract chunks from architecture doc");

        // Count chunk types
        let headings = chunks.iter().filter(|c| c.kind.starts_with("heading_")).count();
        let code_blocks = chunks.iter().filter(|c| c.kind == "code_block").count();

        println!("MD_ENHANCE_ARCHITECTURE.md: Extracted {} chunks ({} headings, {} code blocks)",
            chunks.len(), headings, code_blocks);

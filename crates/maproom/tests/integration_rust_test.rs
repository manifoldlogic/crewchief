...
"#;
    ...
    for chunk in &chunks {
        println!(
            "  - {} ({}) lines {}-{}: {}",
            chunk.symbol_name.as_ref().unwrap_or(&"<anonymous>".to_string()),
            chunk.kind,
            chunk.start_line,
            chunk.end_line,
            chunk.docstring.as_ref().map(|d| &d[..d.len().min(50)]).unwrap_or("")
        );
    }

    // Verify we extracted the major items
    assert!(chunks.iter().any(|c| c.kind == "constant" && c.symbol_name == Some("MAX_RETRIES".to_string())));
    ...
    assert!(chunks.iter().any(|c| c.kind == "enum" && c.symbol_name == Some("State".to_string())));
    assert!(chunks.iter().any(|c| c.kind == "macro" && c.symbol_name == Some("log".to_string())));
    assert!(chunks.iter().any(|c| c.kind == "module" && c.symbol_name == Some("private_module".to_string())));
    ...
    let fetch_data = chunks.iter().find(|c| c.symbol_name == Some("fetch_data".to_string())).unwrap();
    ...
    let impl_functions: Vec<_> = chunks.iter().filter(|c| {
        c.kind == "func" &&
        (c.symbol_name == Some("new".to_string()) || c.symbol_name == Some("get".to_string()))
    }).collect();
    assert!(impl_functions.len() >= 2, "Should have extracted impl functions");

    println!("\n✅ Successfully parsed real Rust file with {} chunks", chunks.len());
}

...
fn benchmark_indexing(c: &mut Criterion) {
    ...
    c.bench_function("sqlite_insert_100_chunks", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let chunks: Vec<ChunkRecord> = (0..100).map(|i| ChunkRecord {
                file_id: 1,
            ...
            store.upsert_file(&FileRecord {
                repo_id: 1, worktree_id: 1, commit_id: 1, relpath: "test.rs".to_string(),
                language: None, content_hash: "h".to_string(), size_bytes: 100, last_modified: None
            }).await.ok();
